...
import 'package:admart_app/pages/transfer_release/transfer_release_detail_item_detail_page.dart';
import 'package:flutter/material.dart';
import 'package:admart_app/bloc_widgets/bloc_state_builder.dart';
import 'package:admart_app/blocs/transfer_release/detail/transfer_release_detail_bloc.dart';
import 'package:admart_app/blocs/transfer_release/detail/transfer_release_detail_event.dart';
import 'package:admart_app/blocs/transfer_release/detail/transfer_release_detail_state.dart';
...
class TransferReleaseDetailPage extends StatefulWidget {
  TransferReleaseDetailPage(this._id);
  final int _id;
  @override
  _TransferReleaseDetailPageState createState() =>
      _TransferReleaseDetailPageState(_id);
}

class _TransferReleaseDetailPageState extends State<TransferReleaseDetailPage> {
  _TransferReleaseDetailPageState(this._id);

  TransferReleaseDetailBloc bloc = TransferReleaseDetailBloc();
  final int _id;
  ...
  void initState() {
    ...
    if (_id != 0) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        bloc.emitEvent(TransferReleaseDetailEventGetId(
          id: _id,
        ));
      });
    } else {
  ...
  void _create() {
    ...
    bloc.emitEvent(TransferReleaseDetailEventAdd(
      data: data,
    ));
  }
  ...
  void _newTrans() {
    MaterialPageRoute newRoute = MaterialPageRoute(
        builder: (BuildContext context) => TransferReleaseDetailPage(0));
            ...
            return AlertDialog(
              ...
              actions: <Widget>[
                FlatButton(
                  child: Text('Ok'),
                  onPressed: () {
                    bloc.emitEvent(TransferReleaseDetailEventNormal());
  ...
  TransferReleaseDetailState _getState() {
  ...
  Future _scanQR() async {
    ...
    try {
      ...
      bloc.emitEvent(TransferReleaseDetailEventScan(
      ...
      //       MaterialPageRoute(
      //         builder: (BuildContext context) =>
      //             TransferReleaseDetailItemDetailPage(onData.newItem),
      //       ),
      //     );

      //     item.then((Item item) {
      //       if (item != null) {
      //         bloc.emitEvent(TransferReleaseDetailEventItemAdd(
      //           item: item,
      //         ));
      //       }
      //     });
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (newItem != null) {
        ...
        Future<Item> item = Navigator.push(
          ...
          MaterialPageRoute(
            builder: (BuildContext context) =>
                TransferReleaseDetailItemDetailPage(newItem),
          ),
        );
  ...
  Widget build(BuildContext context) {
    _context = context;
    var data = _getState().data;
    return BlocEventStateBuilder<TransferReleaseDetailState>(
        bloc: bloc,
        builder: (BuildContext context, TransferReleaseDetailState state) {
          return SafeArea(
            child: Scaffold(
              key: _scaffoldKey,
              appBar: _appBar(),
  ...
  void _showItemDetail(int itemIndex) {
    ...
    Future<Item> item = Navigator.push(
      ...
      MaterialPageRoute<Item>(
        builder: (BuildContext context) =>
            TransferReleaseDetailItemDetailPage(items[itemIndex]),
      ),
    );

    item.then((Item item) {
      if (item != null) {
        bloc.emitEvent(TransferReleaseDetailEventItemUpdate(
          item: item,
          itemIndex: itemIndex,
        ));
      }
  ...
  Widget _buildForm() {
    ...
    //       globalBloc.loginResponse.data.transferRelease_WhsCodeFrom ?? '';
    //   _whsNameFromController.text =
    //       globalBloc.loginResponse.data.transferRelease_WhsNameFrom ?? '';
    //   _whsCodeToController.text =
    //       globalBloc.loginResponse.data.transferRelease_WhsCodeTo ?? '';
