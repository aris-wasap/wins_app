...
class CflBranchPage extends StatefulWidget {
  ...
  final int branchId;
  @override
  _CflBranchPageState createState() => _CflBranchPageState(branchId);
}

class _CflBranchPageState extends State<CflBranchPage> {
  _CflBranchPageState(this.branchId);
  CflBranchBloc bloc = CflBranchBloc();
  ...
  _onSearchChanged() {
    ...
    _debounce = Timer(const Duration(milliseconds: 2000), () {
      bloc.emitEvent(CflBranchEvent(
        event: CflBranchEventType.firstPage,
  ...
  void _onScroll() {
    if (_scrollController.offset ==
      ...
      bloc.emitEvent(CflBranchEvent(
        event: CflBranchEventType.nextPage,
        searchQuery: _searchQueryController.text,
        branchId: branchId,
      ));
    }
  ...
  PreferredSizeWidget _appBar(CflBranchState state) {
        ...
        actions: <Widget>[
          IconButton(
              ...
              onPressed: () {
                ...
                bloc.emitEvent(CflBranchEvent(
                  event: CflBranchEventType.deactivedSearch,
                  searchQuery: _searchQueryController.text,
                  branchId: branchId,
                ));
              }),
      ...
      return AppBar(
        ...
        actions: <Widget>[
          IconButton(
            ...
            onPressed: () {
              bloc.emitEvent(CflBranchEvent(
                event: CflBranchEventType.activedSearch,
                branchId: branchId,
              ));
            },
          ),
  ...
  Future<void> _handleRefresh() async {
    bloc.emitEvent(CflBranchEvent(
      event: CflBranchEventType.refresh,
      searchQuery: _searchQueryController.text,
    ));
  }
  ...
  Widget build(BuildContext context) {
    return BlocEventStateBuilder<CflBranchState>(
        bloc: bloc,
        builder: (BuildContext context, CflBranchState state) {
          return SafeArea(
            child: Scaffold(
              key: _scaffoldKey,
  ...
  Widget buildList(CflBranchState state) {
