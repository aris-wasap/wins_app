...
class _CflPurchaseDeliveryPageState extends State<CflPurchaseDeliveryPage> {
  CflPurchaseDeliveryBloc bloc = CflPurchaseDeliveryBloc();
  ...
  _onSearchChanged() {
    ...
    _debounce = Timer(const Duration(milliseconds: 2000), () {
      bloc.emitEvent(CflPurchaseDeliveryEvent(
        event: CflPurchaseDeliveryEventType.firstPage,
  ...
  void _onScroll() {
    if (_scrollController.offset ==
      ...
      bloc.emitEvent(CflPurchaseDeliveryEvent(
        event: CflPurchaseDeliveryEventType.nextPage,
        searchQuery: _searchQueryController.text,
      ));
    }
  }
  ...
  PreferredSizeWidget _appBar(CflPurchaseDeliveryState state) {
        ...
        actions: <Widget>[
          IconButton(
              ...
              onPressed: () {
                ...
                bloc.emitEvent(CflPurchaseDeliveryEvent(
                  event: CflPurchaseDeliveryEventType.deactivedSearch,
                  searchQuery: _searchQueryController.text,
                ));
              }),
        ],
      ...
      return AppBar(
        ...
        actions: <Widget>[
          IconButton(
            ...
            onPressed: () {
              bloc.emitEvent(CflPurchaseDeliveryEvent(
                event: CflPurchaseDeliveryEventType.activedSearch,
              ));
            },
          ),
        ],
  ...
  Future<void> _handleRefresh() async {
    bloc.emitEvent(CflPurchaseDeliveryEvent(
      event: CflPurchaseDeliveryEventType.refresh,
      searchQuery: _searchQueryController.text,
    ));
  }
  ...
  Widget build(BuildContext context) {
    return BlocEventStateBuilder<CflPurchaseDeliveryState>(
        bloc: bloc,
        builder: (BuildContext context, CflPurchaseDeliveryState state) {
          return SafeArea(
            child: Scaffold(
              key: _scaffoldKey,
  ...
  Widget buildList(CflPurchaseDeliveryState state) {
