...
import 'package:admart_app/pages/transfer_reject/transfer_reject_detail_item_detail_page.dart';
import 'package:flutter/material.dart';
import 'package:admart_app/bloc_widgets/bloc_state_builder.dart';
import 'package:admart_app/blocs/transfer_reject/detail/transfer_reject_detail_bloc.dart';
import 'package:admart_app/blocs/transfer_reject/detail/transfer_reject_detail_event.dart';
import 'package:admart_app/blocs/transfer_reject/detail/transfer_reject_detail_state.dart';
...
class TransferRejectDetailPage extends StatefulWidget {
  TransferRejectDetailPage(this._id);
  final int _id;
  @override
  _TransferRejectDetailPageState createState() =>
      _TransferRejectDetailPageState(_id);
}

class _TransferRejectDetailPageState extends State<TransferRejectDetailPage> {
  _TransferRejectDetailPageState(this._id);

  TransferRejectDetailBloc bloc = TransferRejectDetailBloc();
  final int _id;
  ...
  void initState() {
    ...
    if (_id != 0) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        bloc.emitEvent(TransferRejectDetailEventGetId(
          id: _id,
        ));
      });
    } else {
  ...
  void _create() {
    ...
    if ([null].contains(data.transDate)) {
      ValidateDialogWidget(
          context: context, massage: "Reject Date harus di isi");
      return;
    ...
    bloc.emitEvent(TransferRejectDetailEventAdd(
      data: data,
    ));
  }
  ...
  void _newTrans() {
    MaterialPageRoute newRoute = MaterialPageRoute(
        builder: (BuildContext context) => TransferRejectDetailPage(0));
            ...
            return AlertDialog(
              ...
              actions: <Widget>[
                FlatButton(
                  child: Text('Ok'),
                  onPressed: () {
                    bloc.emitEvent(TransferRejectDetailEventNormal());
  ...
  PreferredSizeWidget _appBar() {
    if (_getState().data.id == 0) {
      ...
      return AppBar(
        ...
        actions: <Widget>[
          (globalBloc.loginResponse.data.transferReject_Auth_Add == 'Y')
              ? IconButton(
                  onPressed: () {
                    _newTrans();
                  },
  ...
  TransferRejectDetailState _getState() {
  ...
  Future _scanQR() async {
    ...
    try {
      ...
      bloc.emitEvent(TransferRejectDetailEventScan(
      ...
      //       MaterialPageRoute(
      //         builder: (BuildContext context) =>
      //             TransferRejectDetailItemDetailPage(onData.newItem),
      //       ),
      //     );

      //     item.then((Item item) {
      //       if (item != null) {
      //         bloc.emitEvent(TransferRejectDetailEventItemAdd(
      //           item: item,
      //         ));
      //       }
      //     });
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (newItem != null) {
        ...
        Future<Item> item = Navigator.push(
          ...
          MaterialPageRoute(
            builder: (BuildContext context) =>
                TransferRejectDetailItemDetailPage(newItem),
          ),
        );
  ...
  Widget build(BuildContext context) {
    _context = context;
    var data = _getState().data;
    return BlocEventStateBuilder<TransferRejectDetailState>(
        bloc: bloc,
        builder: (BuildContext context, TransferRejectDetailState state) {
          return SafeArea(
            child: Scaffold(
              key: _scaffoldKey,
              appBar: _appBar(),
  ...
  void _showItemDetail(int itemIndex) {
    ...
    Future<Item> item = Navigator.push(
      ...
      MaterialPageRoute<Item>(
        builder: (BuildContext context) =>
            TransferRejectDetailItemDetailPage(items[itemIndex]),
      ),
    );

    item.then((Item item) {
      if (item != null) {
        bloc.emitEvent(TransferRejectDetailEventItemUpdate(
          item: item,
          itemIndex: itemIndex,
        ));
      }
  ...
  Widget _buildForm() {
    ...
    //       globalBloc.loginResponse.data.transferReject_WhsCodeFrom ?? '';
    //   _whsNameFromController.text =
    //       globalBloc.loginResponse.data.transferReject_WhsNameFrom ?? '';
    //   _whsCodeToController.text =
    //       globalBloc.loginResponse.data.transferReject_WhsCodeTo ?? '';
        ...
        if (_getState().data.id == 0) {
          return Dismissible(
            ...
            onDismissed: (direction) {
              bloc.emitEvent(TransferRejectDetailEventItemRemove(itemIndex: index));
            },
